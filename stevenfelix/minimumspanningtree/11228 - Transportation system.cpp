...
int main() {
    ...
    d = 1;
    while(t--){
        long double road, rail, dis, mr, tx, ty;
        cin >> n >> mr;
        xy.clear();
        for(i=0;i<n;i++){
            cin >> tx >> ty;
        ...
        for(i=0;i<n;i++){
            for(j=i+1;j<n;j++){
                ...
                if(dis<=mr) q.push({dis, {i+1, j+1}});
                else q2.push({dis, {i+1, j+1}});
            }
        }
        initset(n+1);
        ans = n;
        ...
        while(!q2.empty()){
            if(!issameset(q2.top().second.first, q2.top().second.second)){
                unionset(q2.top().second.first, q2.top().second.second);
                rail+=q2.top().first;
            }
            q2.pop();
        }
        cout << "Case #"<<d++<<": "<<ans <<" "<<(long long)(road+0.5)<<" "<<(long long)(rail+0.5)<<endl;
    }
    return 0;
}
